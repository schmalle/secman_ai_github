...
def _create_issues_sync(
    ...
    findings: list, dry_run: bool, title_prefix: str,
) -> tuple[int, int]:
    ...
    for finding in findings:
        outcome = process_finding(
            ...
            seen_at=_now(), dry_run=dry_run, title_prefix=title_prefix,
        )
        ...
        try:
            ...
            if store is not None and cfg.create_issues and res.high_critical:
                created, skipped = await asyncio.to_thread(
                    ...
                    res.high_critical, cfg.issue_dry_run, cfg.issue_title_prefix,
                )
