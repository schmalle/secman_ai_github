...
Secrets are never CLI flags: `DB_PASSWORD` (state DB) and `SECMAN_PASSWORD` (secman,
an ADMIN/VULN-role credential) are environment-variable-only, like `GITHUB_TOKEN`,
`GITHUB_APP_PRIVATE_KEY[_PATH]` and `SMTP_PASSWORD` — so none of them can land in
argv, `ps`/`/proc/<pid>/cmdline`, shell history, or a CI job's echoed command line.
`--db-user`/`--secman-username`/`--secman-url` are plain identifiers, not secrets,
so they stay available as flags.
"""
...
_PUSH_TO_SECMAN_HELP = (
    ...
    "combine with --no-db. Needs --secman-url/--secman-username plus the SECMAN_PASSWORD "
    "env var (the password is never a CLI flag — see below). Only the repositories "
    "reviewed here are pushed; use `secscan push-to-secman` for the whole state DB."
)
...
# No --secman-password flag by design: this credential grants ADMIN/VULN-role write
# access to secman's vulnerability tracker, so it must never be able to land in argv,
# `ps`/`/proc/<pid>/cmdline`, shell history, or a CI job's echoed command line — only
# SECMAN_PASSWORD (env) is accepted. Same rationale as the argv-free GitHub clone token
# in cloner.py; DB_PASSWORD (state DB) follows the identical rule below.

_SECMAN_CREDS_MISSING = (
    "secman URL/username/password required "
    "(--secman-url/--secman-username, or SECMAN_URL/SECMAN_USERNAME env vars, plus the "
    "SECMAN_PASSWORD env var — there is no --secman-password flag, so the password can "
    "never leak through argv/`ps`)"
)
...
def _run_config(
    ...
    if not push_to_secman and (secman_url or secman_username or secman_password):
        raise ConfigError("--secman-url/--secman-username (or SECMAN_PASSWORD) require --push-to-secman")
...
def run(
    ...
    db_user: str = typer.Option(None, help="MySQL/MariaDB username (or DB_USERNAME env). Overrides any user embedded in --db-url. The password has no CLI flag — set DB_PASSWORD (env) only, so it never lands in argv/`ps`."),
    db_user: str = typer.Option(None, help="MySQL/MariaDB username (or DB_USERNAME env). Overrides any user embedded in --db-url. Password: DB_PASSWORD env only (no --db-password flag, to avoid exposing it via `ps`)."),
    ...
    try:
        cfg = _run_config(
            ...
            # DB and secman passwords are never CLI flags (only DB_PASSWORD / SECMAN_PASSWORD
            # env vars) so a secret never lands in argv, `ps`, shell history, or a CI log —
            # the same argv-safety invariant cloner.py already applies to the GitHub token.
            ...
            secman_username=secman_username, secman_password=None,
...
def list_repos(
    ...
    store = None if no_db or not last_commit else StateStore(
        _resolve_db_url(db_url) or (output_dir / "secscan.sqlite3"),
        db_user=_resolve_db_user(db_user),
        # DB_PASSWORD env var only — no --db-password flag (see run/scan for why).
        db_password=_resolve_db_password(None),
        db_ssl=_resolve_db_ssl(db_ssl),
    )
    try:
...
def list_users(
    ...
    if not no_db:
        store = _open_store(output_dir, db_url, db_user, db_ssl=db_ssl)
...
def review(
    ...
    cfg = _run_config(
        ...
        db_url=_resolve_db_url(db_url), db_user=_resolve_db_user(db_user),
        # DB_PASSWORD env var only — no --db-password flag, so the secret never lands
        # in argv/`ps`/shell history (see the same rationale on run/scan below).
        db_password=_resolve_db_password(None), db_ssl=_resolve_db_ssl(db_ssl),
        db_ssl=_resolve_db_ssl(db_ssl),
        no_db=not store_db,
...
def push_to_secman(
    ...
    output_dir: Path = typer.Option(Path("output"), help="Where the state DB lives."),
    db_url: str = typer.Option(None, help="MySQL/MariaDB URL; defaults to SECSCAN_DB_URL or local SQLite."),
    db_user: str = typer.Option(None, help="MySQL/MariaDB username (or DB_USERNAME env). The password has no CLI flag — set DB_PASSWORD (env) only, so it never lands in argv/`ps`."),
    db_user: str = typer.Option(None, help="MySQL/MariaDB username (or DB_USERNAME env). Password: DB_PASSWORD env only (no --db-password flag, to avoid exposing it via `ps`)."),
    ...
    The password is read from the SECMAN_PASSWORD environment variable only — there
    is no --secman-password flag, so this ADMIN/VULN-role credential never lands in
    argv, `ps`/`/proc/<pid>/cmdline`, shell history, or a CI job's echoed command line.
    """
    ...
    # SECMAN_PASSWORD env var only — no --secman-password flag, so an ADMIN/VULN-role
    # secman credential never lands in argv/`ps`/shell history/a CI job log (see run/scan).
    # No --secman-password flag exists (argv/ps exposure) — password comes from
    # SECMAN_PASSWORD only.
